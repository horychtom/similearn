...
class DenoisingAutoEncoderLoss(nn.Module):
    ...
    def __init__(
        ...
        self.need_retokenization = not (
            type(self.tokenizer_encoder) == type(self.tokenizer_decoder)
        )
        ...
        try:
            ...
            raise e
        assert (
            model[0].auto_model.config.hidden_size
        ...
        if tie_encoder_decoder:
            assert (
                not self.need_retokenization
            ), "The tokenizers should be the same when tie_encoder_decoder=True."
    ...
    def forward(
        ...
        reps = self.encoder(source_features)[
            "sentence_embedding"
        ]  # (bsz, hdim)

        # Prepare input and output
        target_length = target_features["input_ids"].shape[1]
        decoder_input_ids = target_features["input_ids"].clone()[
            :,
            : target_length - 1,
        ]
