...
class PhraseTokenizer(WordTokenizer):
    ...
    def set_vocab(self, vocab: Iterable[str]):
        ...
        for word in vocab:
            if (
                self.ngram_separator is not None
                and self.ngram_separator in word
            ):
        ...
        if len(vocab) > 0:
            logger.info(
                "PhraseTokenizer - Phrase ngram lengths: {}".format(
                    self.ngram_lengths,
                ),
            )
            logger.info(
                "PhraseTokenizer - Num phrases: {}".format(
                    len(self.ngram_lookup),
                ),
            )
    ...
    def tokenize(self, text: str) -> List[int]:
        ...
        for ngram_len in sorted(self.ngram_lengths, reverse=True):
            ...
            while idx <= len(tokens) - ngram_len:
                ngram = self.ngram_separator.join(
                    tokens[idx : idx + ngram_len],
                )
    ...
    def save(self, output_path: str):
        with open(
            os.path.join(output_path, "phrasetokenizer_config.json"),
            "w",
        ) as fOut:
            json.dump(
                {
    ...
    def load(input_path: str):
        with open(
            os.path.join(input_path, "phrasetokenizer_config.json"),
            "r",
        ) as fIn:
