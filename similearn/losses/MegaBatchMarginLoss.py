...
class MegaBatchMarginLoss(nn.Module):
    ...
    def forward_mini_batched(
        ...
        with torch.no_grad():
            self.model.eval()
            all_positive_emb = self.model(positive)[
                "sentence_embedding"
            ].detach()
            self.model.train()

        diagonal_matrix = torch.eye(
            len(all_positive_emb),
            len(all_positive_emb),
            device=all_positive_emb.device,
        )
        ...
        for start_idx in range(0, len(all_positive_emb), self.mini_batch_size):
            ...
            with torch.no_grad():
                ...
                negatives_max, negatives_ids = torch.max(
                    negative_scores,
                    dim=1,
                )
            ...
            for hard_negative_id in negatives_ids:
                for key in feature_names:
                    hard_negative_features[key].append(
                        positive[key][hard_negative_id],
                    )

            for key in feature_names:
                hard_negative_features[key] = torch.stack(
                    hard_negative_features[key],
                )

            # Compute differentiable negative and positive embeddings
            positive_emb = self.model(
                {
                    key: positive[key][start_idx:end_idx]
                    for key in feature_names
                },
            )["sentence_embedding"]
            negative_emb = self.model(hard_negative_features)[
                "sentence_embedding"
            ]
