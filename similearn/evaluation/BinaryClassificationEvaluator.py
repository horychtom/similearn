...
class BinaryClassificationEvaluator(SentenceEvaluator):
    ...
    def __call__(
        ...
        logger.info(
            "Binary Accuracy Evaluation of the model on "
            + self.name
            + " dataset"
            + out_txt,
        )
    ...
    def compute_metrices(self, model):
        ...
        for short_name, name, scores, reverse in [
            ...
            (
                ...
                recall,
                f1_threshold,
            ) = self.find_best_f1_and_threshold(scores, labels, reverse)
            ...
            logger.info(
                "Accuracy with {}:           {:.2f}\t(Threshold: {:.4f})".format(
                    name,
                    acc * 100,
                    acc_threshold,
                ),
            ...
            logger.info(
                "F1 with {}:                 {:.2f}\t(Threshold: {:.4f})".format(
                    name,
                    f1 * 100,
                    f1_threshold,
                ),
            ...
            logger.info(
                "Precision with {}:          {:.2f}".format(
                    name,
                    precision * 100,
                ),
            )
            logger.info(
                "Recall with {}:             {:.2f}".format(
                    name,
                    recall * 100,
                ),
            )
    ...
    def find_best_acc_and_threshold(
        ...
        rows = sorted(
            rows,
            key=lambda x: x[0],
            reverse=high_score_more_similar,
        )
