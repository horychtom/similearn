...
import logging
from similearn.util import paraphrase_mining
import os
import csv
...
class ParaphraseMiningEvaluator(SentenceEvaluator):
    ...
    def __init__(
        ...
        for key1 in self.duplicates:
            for key2 in self.duplicates[key1]:
                if (
                    ...
                    and (
                        self.duplicates[key1][key2]
                        or self.duplicates[key2][key1]
                    )
                ):
        ...
        self.csv_file: str = (
            "paraphrase_mining_evaluation" + name + "_results.csv"
        )
        self.csv_headers = [
            "epoch",
            "steps",
    ...
    def __call__(
        ...
        logger.info(
            "Paraphrase Mining Evaluation on "
            + self.name
            + " dataset"
            + out_txt,
        )
        ...
        logger.info(
            "Average Precision: {:.2f}".format(average_precision * 100),
        )
