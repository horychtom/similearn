from similearn.evaluation import SentenceEvaluator
from similearn.wandb_ import WandbClient
import torch
from torch import Tensor
import logging
from tqdm import trange
from similearn.util import cos_sim
import os
import numpy as np
...
class InformationRetrievalEvaluator(SentenceEvaluator):
    ...
    def __init__(
        ...
        accuracy_at_k: List[int] = [3, 5, 10],
        precision_recall_at_k: List[int] = [3, 5, 10],
    ...
    def __call__(
        ...
        **kwargs,
        ...
        return scores[self.eval_metric][max(self.output_k)]
    ...
    def compute_metrices(
        ...
        for corpus_start_idx in trange(
            ...
            pair_scores = self.score_function(
                query_embeddings,
            ...
            for query_itr in range(len(query_embeddings)):
                for sub_corpus_id, score in zip(
                    ...
                    corpus_id = self.corpus_ids[
                        corpus_start_idx + sub_corpus_id
                    ]
                    if len(queries_result_list[query_itr]) < max_k:
                        heapq.heappush(
        ...
        for query_itr in range(len(queries_result_list)):
            for doc_itr in range(
                len(queries_result_list[query_itr]),
            ):
    ...
    def compute_metrics(self, queries_result_list: List[object]):
        ...
        f1_at_k = {k: [] for k in self.precision_recall_at_k}
    ...
    def log_scores(self, scores):
        ...
        for k in scores["accuracy@k"]:
            report_dict["accuracy@{}".format(k)] = scores["accuracy@k"][k]

        for k in scores["precision@k"]:
            report_dict["precision@{}".format(k)] = scores["precision@k"][k]
            report_dict["recall@{}".format(k)] = scores["recall@k"][k]
            report_dict["f1@{}".format(k)] = scores["f1@k"][k]

        for k in scores["mrr@k"]:
            report_dict["MRR@{}".format(k)] = scores["mrr@k"][k]

        for k in scores["ndcg@k"]:
            report_dict["NDCG@{}".format(k)] = scores["ndcg@k"][k]

        for k in scores["map@k"]:
            report_dict["MAP@{}".format(k)] = scores["map@k"][k]

        self.wandbc.run.log(report_dict)

    @staticmethod
    def compute_dcg_at_k(relevances, k):
        dcg = 0
        for i in range(min(len(relevances), k)):
            dcg += relevances[i] / np.log2(
                i + 2,
            )  # +2 as we start our idx at 0
        return dcg
