...
from similearn.util import cos_sim, dot_score
import os
import numpy as np
...
class InformationRetrievalEvaluator(SentenceEvaluator):
    ...
    def __init__(
        ...
        self.csv_file: str = (
            "Information-Retrieval_evaluation" + name + "_results.csv"
        )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.main_score_function is None:
            ...
            return scores[self.main_score_function]["map@k"][
                max(self.map_at_k)
            ]
        ...
        for name in queries_result_list:
            for query_itr in range(len(queries_result_list[name])):
                for doc_itr in range(
                    len(queries_result_list[name][query_itr]),
                ):
                    score, corpus_id = queries_result_list[name][query_itr][
                        doc_itr
                    ]
    ...
    def output_scores(self, scores):
        ...
        for k in scores["precision@k"]:
            logger.info(
                "Precision@{}: {:.2f}%".format(
                    k,
                    scores["precision@k"][k] * 100,
                ),
            )

        for k in scores["recall@k"]:
            logger.info(
                "Recall@{}: {:.2f}%".format(k, scores["recall@k"][k] * 100),
            )
