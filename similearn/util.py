    ...
    for corpus_start_idx in range(0, len(embeddings), corpus_chunk_size):
        for query_start_idx in range(0, len(embeddings), query_chunk_size):
            ...
            for query_itr in range(len(scores)):
                for top_k_idx, corpus_itr in enumerate(
                    scores_top_k_idx[query_itr],
                ):
                    ...
                    if (
                        i != j
                        and scores_top_k_values[query_itr][top_k_idx]
                        > min_score
                    ):
                        pairs.put(
                            (scores_top_k_values[query_itr][top_k_idx], i, j),
                        )
                        num_added += 1
...
def community_detection(
    ...
    for start_idx in range(0, len(embeddings), batch_size):
        ...
        cos_scores = cos_sim(
            embeddings[start_idx : start_idx + batch_size],
            embeddings,
        )
...
def snapshot_download(
    ...
    for model_file in all_files:
        ...
        if version.parse(huggingface_hub.__version__) >= version.parse(
            "0.8.1",
        ):
