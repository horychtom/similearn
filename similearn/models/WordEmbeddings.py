...
import os
import json
from similearn.util import import_from_string, fullname, http_get
...
class WordEmbeddings(nn.Module):
    ...
    def save(self, output_path: str):
        with open(
            os.path.join(output_path, "wordembedding_config.json"),
            "w",
        ) as fOut:
    ...
    def load(input_path: str):
        with open(
            os.path.join(input_path, "wordembedding_config.json"),
            "r",
        ) as fIn:
    ...
    def from_text_file(
        ...
        if not os.path.exists(embeddings_file_path):
            logger.info(
                "{} does not exist, try to download from server".format(
            ...
            if "/" in embeddings_file_path or "\\" in embeddings_file_path:
                raise ValueError(
                    "Embeddings file not found: ".format(embeddings_file_path),
                )
        ...
        with gzip.open(
            ...
            "rt",
            encoding="utf8",
        ) if embeddings_file_path.endswith(".gz") else open(
            embeddings_file_path,
            encoding="utf8",
        ) as fIn:
            iterator = tqdm(
                fIn,
                desc="Load Word Embeddings",
                unit="Embeddings",
            )
