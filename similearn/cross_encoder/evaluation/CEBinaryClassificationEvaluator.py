...
from similearn.evaluation import BinaryClassificationEvaluator
...
class CEBinaryClassificationEvaluator:
    ...
    def __call__(
        ...
        logger.info(
            "CEBinaryClassificationEvaluator: Evaluating the model on "
            + self.name
            + " dataset"
            + out_txt,
        )
        ...
        (
            acc,
            acc_threshold,
        ) = BinaryClassificationEvaluator.find_best_acc_and_threshold(
            pred_scores,
            self.labels,
            True,
        )
        (
            ...
            recall,
            f1_threshold,
        ) = BinaryClassificationEvaluator.find_best_f1_and_threshold(
            pred_scores,
            self.labels,
            True,
        )
        ...
        logger.info(
            "Accuracy:           {:.2f}\t(Threshold: {:.4f})".format(
                acc * 100,
                acc_threshold,
            ),
        )
        logger.info(
            "F1:                 {:.2f}\t(Threshold: {:.4f})".format(
                f1 * 100,
                f1_threshold,
            ),
        )
