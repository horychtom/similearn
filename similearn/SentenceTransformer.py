...
class SentenceTransformer(nn.Sequential):
    ...
    :param cache_folder: Path to store models. Can be also set by SIMILEARN_HOME enviroment variable.
    ...
    def __init__(
        ...
        if cache_folder is None:
            cache_folder = os.getenv("SIMILEARN_HOME")
            if cache_folder is None:
                try:
        ...
        if model_name_or_path is not None and model_name_or_path != "":
            ...
            if os.path.exists(model_name_or_path):
                ...
                if (
                    ...
                    and model_name_or_path.lower()
                    not in basic_transformer_models
                ):
                    # A model from similearn
                    model_name_or_path = (
    ...
    def encode_multi_process(
        ...
        if chunk_size is None:
            chunk_size = min(
                math.ceil(len(sentences) / len(pool["processes"]) / 10),
                5000,
            )
        ...
        results_list = sorted(
            [output_queue.get() for _ in range(last_chunk_id)],
            key=lambda x: x[0],
        )
    ...
    def save(
        ...
        if "__version__" not in self._model_config:
            self._model_config["__version__"] = {
                "similearn": __version__,
                "transformers": transformers.__version__,
                "pytorch": torch.__version__,
            }

        with open(
            os.path.join(path, "config_similearn.json"),
            "w",
        ) as fOut:
    ...
    def save_to_hub(
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            if len(large_files) > 0:
                logger.info(
                    "Track files with git lfs: {}".format(
                        ", ".join(large_files),
                    ),
                )
    ...
    def fit(
        ...
        :param evaluator: An evaluator (similearn.evaluation) evaluates the model performance during training on held-out dev data. It is used to determine the best model that is saved to disc.
    ...
    def _save_checkpoint(
        ...
        if (
            checkpoint_save_total_limit is not None
            and checkpoint_save_total_limit > 0
        ):
            old_checkpoints = []
    ...
    def _load_auto_model(self, model_name_or_path):
        ...
        """
        logger.warning(
            "No similearn model found with name {}. Creating a new one with MEAN pooling.".format(
                model_name_or_path,
            ),
        )
    ...
    def _load_sbert_model(self, model_path):
        """
        Loads a full similearn model
        """
        # Check if the config_similearn.json file exists (exists since v2 of the framework)
        config_similearn_json_path = os.path.join(
            model_path,
            "config_similearn.json",
        )
        if os.path.exists(config_similearn_json_path):
            with open(config_similearn_json_path) as fIn:
