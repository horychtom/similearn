from similearn import util, SentenceTransformer
import unittest
import numpy as np
import sklearn
import torch
...
class UtilTest(unittest.TestCase):
    ...
    def test_pytorch_cos_sim(self):
        ...
        for i in range(len(sklearn_pairwise)):
            for j in range(len(sklearn_pairwise[i])):
                assert (
                    abs(sklearn_pairwise[i][j] - pytorch_cos_scores[i][j])
                    < 0.001
                )
    ...
    def test_semantic_search(self):
        ...
        for qid in range(num_queries):
            for hit_num in range(num_k):
                assert (
                    hits[qid][hit_num]["corpus_id"]
                    == cos_scores_idx[qid][hit_num]
                )
                assert (
                    np.abs(
                        hits[qid][hit_num]["score"]
                        - cos_scores_values[qid][hit_num],
                    )
                    < 0.001
                )
    ...
    def test_pairwise_scores(self):
        ...
        # Pairwise cos
        sklearn_pairwise = (
            1 - sklearn.metrics.pairwise.paired_cosine_distances(a, b)
        )
